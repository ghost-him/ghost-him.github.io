...
为什么要内存对齐？
+ 有的硬件无法访问任意地址上的任意数据
+ 性能原因，CPU从内存中读取数据是按存储块读取的，比如4，6，16字节这样。在对齐情况下，只需要1次总结周期就可以完成4字节的读取。如果是非对齐，那么可能需要两次内存访问，同时还执行了额外的位移与拼接操作，这会降低吞吐量。
...
函数在调用时，会进行以下的压栈操作：
+ 传递参数：调用者（Caller）先将参数压栈。
+ 保存返回地址：执行 CALL 指令时，CPU 自动将下一条指令的地址压栈。
+ 保存调用者的栈帧指针：进入函数后，函数内部（Callee）将旧的栈帧基址（如 EBP/RBP）压栈。
+ 分配局部变量空间：通过移动栈指针（如 ESP/RSP）为局部变量预留空间。
...
+ 分配内存的位置：malloc 是在堆上动态分配内存，而 new 是从自由存储区为对象动态分配内存。同时，自由存储区的位置取决于 `operator new` 的实现。因此，自由存储区不仅可以是堆，也可以是静态存储区。
+ 返回类型安全性：malloc 内存分配成功后返回 `void *`，然后再*强制类型转换*为需要的类型；而new操作符分配内存成功后直接返回与对象类型相匹配的指针类型；所以 new 是符合类型安全的操作符。
+ 内存分配失败返回值：malloc 内存分配失败后返回 NULL。new分配内存失败则抛出异常（`std::bad_alloc`）。
+ 分配内存的大小的计算：使用new操作符时，不需要指定内存块的大小，编译器会根据类型信息自行计算，而malloc 需要显式的指出所需要的内存的尺寸。
+ 重载：`new` 与 `delete`可以被重载，而 `malloc` 与 `free` 无法被重载。
+ 初始化：`new` 在成功分配到内存后，会直接调用构造函数。而free则需要自己手动完成内存的初始化。

== malloc 的原理与底层实现

#{
  tufted.margin-note(image("asset/brk.webp"))
}


原理：
+ 当开辟的空间小于 128K 时，调用 brk()函数，通过移动 program break 来实现；
+ 当开辟的空间大于 128K 时，调用 mmap() 函数，䏍在虚拟地址空间中开辟一块内存空间来实现。

在现代的操作系统中，这个128K的阈值是动态调整的。如果开启了动态阈值，它会根据分配习惯在128K到32MB之间变动。

#{
  tufted.margin-note(image("asset/mmap.webp"))
}

`brk()` 原理：会将堆顶指针向高地址移动，从而获得新的内存空间。
`mmap()` 原理：通过匿名映射，从文件映射区中分配一块内存。
