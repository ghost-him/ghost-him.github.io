...
一致性算法允许一组机器作为一个协调的整体工作，从而在部分成员故障时仍能正常运行。因此它们在构建可靠的大规模软件系统中起着关键作用。
...
+ 强领导者：Raft 使用比其他一致性算法更强的领导形式。例如，日志条目仅从领导者流向其他服务器。这简化了复制日志的管理，并使 Raft 更易于理解。
 领导者选举：Raft 使用随机定时器来选举领导者。这在任何一致性算法已要求的“心跳”机制的基础上仅增加了少量额外机制，同时能简单快速地解决冲突。
...
一致性算法通常出现在需要重复状态机的背景下。如果一组拥有相同状态机的服务器出现了少部分故障，那么它们依然能继续运行。
...
Raft 通过首先选举一个领导者，然后赋予该领导者管理复制日志的全权来实现共识。领导者从客户端接收日志条目，将其复制到其他服务器上，并在安全性允许时告知服务器将日志条目应用到其状态机中。拥有领导者简化了复制日志的管理。领导者可能会失效或失去与其他服务器的连接，在这种情况下会选举产生新的领导者
...
不同的服务器可能会在不同的时间观察到任期之间的转换，并且在某些情况下，服务器无法观察到某次选举甚至整体任期。为了解决这种问题，在 Raft 中，选择了*任期*充当了逻辑时钟，它们允许服务器检测过时的信息#footnote[比如，过时的领导者]。每台服务器都存储一个当前的任期号，该编号随时间单调递增。每当服务器通信时，都会交换当前的任期；如果一个服务器的当前任期小于另一个服务器，则它会将其当前任期更新为较大的值。如果候选人或领导都发现其任期已经过时，它会立即转回跟随者状态。如果服务器收到带有过时任期号的请求#footnote[这里是指过时的领导者的请求（复制日志与发送心跳等）以及过时的候选人的请求（在选举期间拉票等）]，它会拒绝该请求。

Raft 服务器使用远程过程调用（RPC）进行通信，基本的一致性算法只需要两种类型的RPC：
- RequestVote RPC：由候选人在选举期间发起
- AppendEntires RPC：由领导者发起，用于复制日志条目并提供一种心跳形式
...
如果一个候选人在同一任期内获得了集群中大多数服务器的选票，它就赢得了选举。在给定的任期内，每台服务器按照先来先服务的原则最多为一个候选人投票。*多数派规则确保了在特定任期内最多只有一个候选人可以赢得选举*。一旦候选人赢得了选举，它就成为了领导者。然后，它会向所有其他服务器发送心跳消息，以确立其权威并降低新一轮选举发生的可能性。

在等待选举期间，候选人可能会收到来自另一个声称是领导者的服务器的 AppendEntires RPC。如果该领导者的任期至少与候选人的当前任期一样大，则候选人承认该领导者的合法性并返回到跟随者的状态。如果 RPC 中的任期小于候选人的当前任期，则候选人拒绝该 RPC 并继续保持候选人状态。

第三种可能的结果是候选人既没有赢得也没有输掉选举：如果许多跟随者同时成为候选人，选票可能会被瓜分，导致没有任何候选人获得多数票。当发生这种情况时，每个候选人都会重新发起选举，并通过增加任期和发起另一轮 RequestVote RPC 来开始新的选举。为了防止这种情况再次重复，Raft 使用随机选举超时来减小选票瓜分的情况再次出现。
...
当领导者被选出后，它就开始为客户端请求提供服务。每个客户端请求都包含一个要由复制状态机执行的命令。领导者将该命令作为一个新的条目追加到日志中，然后并行地向其他每个服务器发出 AppendEntries RPC，以复制该条目。当该条目被安全地复制后，领导者将该条目应用到其状态机中，并将执行结果返回给客户端。如果跟随者崩溃或运行缓慢，或者网络数据包丢失，领导都会无限期地重试 AppendEntires RPC（甚至在它已经响应客户端后），直到所有跟随者最终存储了所有的日志条目。#footnote[我写代码实现这部分的时候，对这个有些误解，如果是RPC错误，也就是网络错误，会让 sendAppendEntries 返回 false, 而对于其他的情况，比如日志不一致这些，它本身的 sendAppendEntires 函数是会返回 true 的，但是 reply 中的语义会返回 false，这里需要区别一下]
...
前面的章节描述了 Raft 如何选举领导者和复制日志条目。然而，到目前为止描述的机制还不足以确保每个状态机以相同的顺序执行完全相同的命令#footnote[例如，当领导者提交多个日志条目时，某个跟随者可能不可用，随后它可能被选为领导者并用新条目覆盖这些条目；结果，不同的状态机可能会执行不同的命令序列。]。
...
这个任务B主要的内容就是实现日志的复制。日志的复制主要有两个任务：
...
至于传入 applyCh 的方式，我个人推荐使用 channel 通道来完成传入。新开一个协程，通道用于通过applyCh来提交协程。如果是各个协程单独提交，很有可能会出现传入的顺序不一致的情况，通过 channel 传入，可以保证顺序上传，同时还是单线程上传，这也是线程安全的。
