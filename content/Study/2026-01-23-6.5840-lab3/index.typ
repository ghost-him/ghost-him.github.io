...
如右图所示，领导者使用一种名为 InstallSnapshot 的新RPC向落后太多的跟随者发送快照。可以从右图中看到其具体的定义。当跟随者通过该RPC接收到快照时，它必须决定如何处理其现有的日志条目。通常，快照会包含接收者日志中尚未包含的新信息。在这种情况下，跟随者需要丢弃整个日志；这些日志都被快照取代，可能存在与快照冲突的未提交条目。如果跟随者收到的快照描述的是其日志的前缀（由于重传或误发），那么快照涵盖条目会被删除，但快照*之后的条目仍然有效*，必须保留。
